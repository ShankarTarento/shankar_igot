...
class DiscussRepository with ChangeNotifier {
      ...
      _recentDiscussions = [
        for (final item in recentDiscussionInfo['topics'])
          Discuss.fromJson(item,
              topicCount: recentDiscussionInfo['topicCount'],
              nextStart: recentDiscussionInfo['nextStart'])
      ];
      var filtered = _recentDiscussions.where(
          (element) => ((element.user!['uid'] != 0 && element.cid != 1)));
  ...
  Future<dynamic> getMyDiscussions() async {
    var recentDiscussionInfo;
    try {
      recentDiscussionInfo = await DiscussService.getMyDiscussion();
    } catch (_) {
      return null;
    }

    return recentDiscussionInfo;
  }
  ...
  Future<dynamic> getMyBestDiscussions() async {
    try {
      ...
      if (recentDiscussionInfo['bestPosts'] != null &&
          recentDiscussionInfo['bestPosts'].isNotEmpty) {
        _discussions = [
  ...
  Future<dynamic> getUpvotedDiscussions() async {
    try {
      ...
      if (upvotedDiscussionInfo['posts'] != null &&
          upvotedDiscussionInfo['posts'].isNotEmpty) {
        _discussions = [
  ...
  Future<dynamic> getDownvotedDiscussions() async {
    try {
      ...
      if (downvotedDiscussionInfo['posts'] != null &&
          downvotedDiscussionInfo['posts'].isNotEmpty) {
        _discussions = [
  ...
  Future<DiscussionDetail?> getDiscussionById(tid, pageNo, sort) async {
    try {
  ...
  Future<dynamic> saveDiscussion(
    ...
    try {
      final discussionDetailInfo =
          await DiscussService.postDiscussion(pid, cid!, title, content, tags);
      _replyDiscussionResponse = discussionDetailInfo['code'];
    } catch (_) {
      return _;
    }
