...
class EventCompleteCertificate extends StatefulWidget {
  final String? name;
  ...
  const EventCompleteCertificate(
      {Key? key,
      this.certificate,
      this.name,
      this.identifier,
  ...
  @override
  State<EventCompleteCertificate> createState() =>
      _EventCompleteCertificateState();
}

class _EventCompleteCertificateState extends State<EventCompleteCertificate> {
  ...
  String formatDate(dynamic date) {
    try {
      if (date is String) {
        ...
        throw FormatException("endDate must be a String");
      }
    } catch (e) {
      return "";
    }
                                        ...
                                        children: [
                                          ...
                                          if (widget.completedOn != null)
                                            Padding(
                                              ...
                                              child: Text(
                                                '${AppLocalizations.of(context)!.mStaticYouEventCertificateCompleted} ${DateTimeHelper.getDateTimeInFormat("${DateTime.fromMillisecondsSinceEpoch(widget.completedOn!)}", desiredDateFormat: IntentType.dateFormat2)}',
  ...
  void downloadClicked() async {
    _generateInteractTelemetryData(
        ((widget.issuedCertificates ?? []).isNotEmpty
  ...
  Future<void> _saveAsPdf(String courseName) async {
    ...
    try {
      ...
      if (certificatePrintUri == null) {
        String certificateId = ((widget.issuedCertificates ?? []).isNotEmpty
            ? widget.issuedCertificates![0]["identifier"]
            : '');
